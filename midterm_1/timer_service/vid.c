...
int kps(char *line, char *s, int i)
{
  while(*s){
    kpc(line, *s);
    s++; line++; i++;
...
int kpuhelp(char *number, int x)
{
  ...
  c = tab[x % 10];
  i = kpuhelp(number, x/10);

  number[i] = c;
  //kprintf("number = %s, i = %d, c = %c\n", number, i, c);
  return ++i;
}

int kpu(char *line, int x, int i)
{
  char number[10];
  int k = 0, j;
...
int kpf(char *s, char *fmt,...)
{
  int *ip, i = 0;
  char *cp, line[128], *str, c;
  ...
  while(*cp){
    ...
    switch(*cp){
    case 'c': kpc(&line[i], (char)*ip); break;
    case 's': i = kps(&line[i],(char *)*ip, i)-1; break;
    case 'd': i = kpi(&line[i], *ip, i); break;
    case 'u': i = kpu(&line[i], *ip, i); break;
    //case 'x': kprintx(*ip);          break;
    }
    cp++; ip++; i++;
  }
